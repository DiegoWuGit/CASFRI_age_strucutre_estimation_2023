...
----- (this script not for BC region, where boreal and hemiboreal regions are seperately treated)
------------------------------------------------------------------------------------------------------------------------------
...
CREATE TABLE diego_test.FNSK_CASFRI_changedscas_fullf_age9015 AS
...
CASE
    ...
    ELSE NULL
	END AS second_cat 	
FROM diego_test.FNSK_CASFRI_changedscas_fullf as fullf
LEFT JOIN diego_test.FNSK_CASFRI_change_species_age as age
...
SELECT first_cat, second_cat, count(*), ROUND((COUNT(*) * 100.0) / SUM(COUNT(*)) OVER(), 2) AS percentage
FROM diego_test.FNSK_CASFRI_changedscas_fullf_age9015
where second_stand_photo_year > first_stand_photo_year +5
AND  NOT(LEFT(second_cas_id, 4) IN ('MB04', 'MB01', 'MB02') OR LEFT(first_cas_id, 4)  IN ('MB04', 'MB01', 'MB02'))  -- for removing error MB area
...
ALTER TABLE
	diego_test.FNSK_CASFRI_changedscas_fullf_age9015
ADD
...
UPDATE diego_test.FNSK_CASFRI_changedscas_fullf_age9015
SET Age_1990 = 
  CASE
    -- LYR->LYR 
    WHEN first_cat LIKE 'LYR' AND second_cat LIKE 'LYR' AND prov LIKE 'MB' AND first_origin_upper =-9999 AND (first_species_1 LIKE 'PICE_MAR%' OR first_species_1 LIKE 'PINU_BAN%') THEN 1990 - (second_origin_upper + second_origin_lower) / 2.0      ------ Old FRI estimations on oldest trees not used, instead we use recent estimation on same area
...
SELECT xy_id, first_cat, first_species_1, first_stand_photo_year, second_cat, second_species_1, second_stand_photo_year, first_origin_upper, first_origin_lower, second_origin_upper, second_origin_lower, species_mean, age_1990,
dist_year_wnw, dist_year_canlad, dist_casfri_year_1, dist_nfdb_year_1, dist_nfdb_year_2, dist_nfdb_year_3, dist_nfdb_year_4,
dist_nfdb_year_5, dist_nfdb_year_6, dist_nfdb_year_7, dist_nfdb_year_8, dist_nfdb_year_9
FROM diego_test.FNSK_CASFRI_changedscas_fullf_age9015
where first_cat LIKE 'NFL' AND second_cat LIKE 'LYR'
AND second_stand_photo_year > first_stand_photo_year +5
AND first_stand_photo_year >=1990  -- special check in base case NFL -> LYR, if disturbance records are involved
AND species_mean IS NOT NULL           -- set age_1990 to NULL if species_mean is still NULL
...
SELECT first_cat, first_stand_photo_year, second_cat, second_stand_photo_year, first_origin_upper, first_origin_lower, second_origin_upper, second_origin_lower,
...
FROM diego_test.FNSK_CASFRI_changedscas_fullf_age9015
where second_stand_photo_year > first_stand_photo_year +5 AND Age_1990 >0
GROUP BY age_1990
...
--3.0 categories and disturbance work
SELECT first_cat, first_species_1, first_stand_photo_year, second_cat, second_species_1, second_stand_photo_year, first_origin_upper, first_origin_lower, second_origin_upper, second_origin_lower, species_mean,
age_1990, age_2015, dist_year_wnw, dist_year_canlad, dist_casfri_year_1, dist_nfdb_year_1, dist_nfdb_year_2, dist_nfdb_year_3, dist_nfdb_year_4,
dist_nfdb_year_5, dist_nfdb_year_6, dist_nfdb_year_7, dist_nfdb_year_8, dist_nfdb_year_9
FROM diego_test.FNSK_CASFRI_changedscas_fullf_age9015
where first_cat LIKE 'LYR' AND second_cat LIKE 'NFL'
AND second_stand_photo_year > first_stand_photo_year +5
AND second_stand_photo_year <2015  -- special check in base case NFL -> LYR, if disturbance records are involved
AND age_2015 > 0           
ORDER BY age_2015 ASC
...
-- 3.3.2 possibility of tree died between 2 epoques

SELECT age_1990, age_2015, first_cat, first_stand_photo_year, second_cat, second_stand_photo_year, first_origin_upper, first_origin_lower, second_origin_upper, second_origin_lower,
	dist_year_wnw, dist_year_canlad, dist_casfri_year_1, dist_casfri_type_1, dist_nfdb_year_1, dist_nfdb_year_2, dist_nfdb_year_3, dist_nfdb_year_4, dist_nfdb_year_5
	species_mean
FROM diego_test.FNSK_CASFRI_changedscas_fullf_age9015
where first_cat LIKE 'LYR' AND second_cat LIKE 'LYR' AND age_2015 <> -9999
AND (age_1990-age_2015)> 0 
AND second_stand_photo_year > first_stand_photo_year +5 
ORDER BY first_stand_photo_year ASC
LIMIT 500
...
--------------------------------------------------------------------------------------------------------------------------------------
---3.4 add a column to hold age_2015_noLogging, using age_2015 calculating method, only without the interference of disturbance records 
--------------------------------------------------------------------------------------------------------------------------------------
-- construct a Only-Fire (No Harvest) scenario for estimating the age structure
...
------------------------------------------------------------------------------------------------------
---3.5 making plots on age_1990, age_2015, and age_2015_noLogging
------------------------------------------------------------------------------------------------------
...
------------------------------------------------------------------------------------------------------
-- 3.6 mean and sd on age_1990, age_2015

SELECT
  round(AVG(age_1990),2) AS average_age,
  round(STDDEV(age_1990),2) AS sd_age
FROM (
  SELECT age_1990
  FROM diego_test.FNBC_CASFRI_changedscas_fullf_hemi_age9015_ten
  WHERE second_stand_photo_year > first_stand_photo_year + 5
	    --AND rocky_side LIKE 'EAST'
) AS combined_data


------------------------------------------------------------------------------------------------------
---3.6a. Update calculation of age2015_nologging
------------------------------------------------------------------------------------------------------

UPDATE diego_test.FNSK_CASFRI_changedscas_fullf_age9015
SET age_2015_noLogging = 
  CASE
    -- LYR->LYR, if age_1990 +25 >= age_2015, 1st step, ‘backward hypothetical period’
    WHEN first_cat LIKE 'LYR' AND second_cat LIKE 'LYR' AND age_1990 +25 >= age_2015 THEN 
	     CASE 
	         WHEN (dist_year_wnw >= 1990 and dist_year_wnw <= 2015 and dist_type_wnw LIKE 'CUT') OR (dist_year_canlad >= 1990 and dist_year_canlad <= 2015 and dist_year_canlad =2) OR (dist_casfri_year_1 >= 1990 and dist_casfri_year_1 <= 2015 and dist_casfri_type_1 LIKE 'CUT') OR (dist_casfri_year_2 >= 1990 and dist_casfri_year_2 <= 2015 and dist_casfri_type_2 LIKE 'CUT')
	         THEN age_1990 +25
	           -- else, ‘foreward hypothetical period’
		 ELSE
		  CASE 
	          WHEN (dist_year_wnw IS NULL OR dist_year_wnw > 2015 OR dist_year_wnw < second_stand_photo_year OR dist_type_wnw LIKE 'CUT') AND (dist_year_canlad IS NULL OR dist_year_canlad > 2015 OR dist_year_canlad < second_stand_photo_year OR dist_year_canlad =2) AND (dist_casfri_year_1 IS NULL OR dist_casfri_year_1 > 2015 OR dist_casfri_year_1 < second_stand_photo_year OR (dist_casfri_type_1 NOT LIKE 'BURN')) AND
		  (dist_nfdb_year_1 IS NULL OR dist_nfdb_year_1 > 2015 OR dist_nfdb_year_1 < second_stand_photo_year) AND (dist_nfdb_year_2 IS NULL OR dist_nfdb_year_2 > 2015 OR dist_nfdb_year_2 < second_stand_photo_year) AND (dist_nfdb_year_3 IS NULL OR dist_nfdb_year_3 > 2015 OR dist_nfdb_year_3 < second_stand_photo_year) AND
		  ...
		  (dist_nfdb_year_7 IS NULL OR dist_nfdb_year_7 > 2015 OR dist_nfdb_year_7 < second_stand_photo_year) AND (dist_nfdb_year_8 IS NULL OR dist_nfdb_year_8 > 2015 OR dist_nfdb_year_8 < second_stand_photo_year) AND (dist_nfdb_year_9 IS NULL OR dist_nfdb_year_9 > 2015 OR dist_nfdb_year_9 < second_stand_photo_year) THEN
   		   ...
   		   CASE WHEN dist_nfdb_year_7 <= 2015 AND dist_nfdb_year_7 >= second_stand_photo_year THEN dist_nfdb_year_7 ELSE NULL END,
	           ...
	           END
             END
    -- LYR->LYR, if age_1990 +25 < age_2015, 2ns step, ‘foreward hypothetical period’
    WHEN first_cat LIKE 'LYR' AND second_cat LIKE 'LYR' AND age_1990 +25 < age_2015 THEN
	     CASE 
	     WHEN (dist_year_wnw IS NULL OR dist_year_wnw > 2015 OR dist_year_wnw < second_stand_photo_year OR dist_type_wnw LIKE 'CUT') AND (dist_year_canlad IS NULL OR dist_year_canlad > 2015 OR dist_year_canlad < second_stand_photo_year OR dist_year_canlad =2) AND (dist_casfri_year_1 IS NULL OR dist_casfri_year_1 > 2015 OR dist_casfri_year_1 < second_stand_photo_year OR (dist_casfri_type_1 NOT LIKE 'BURN')) AND
		  (dist_nfdb_year_1 IS NULL OR dist_nfdb_year_1 > 2015 OR dist_nfdb_year_1 < second_stand_photo_year) AND (dist_nfdb_year_2 IS NULL OR dist_nfdb_year_2 > 2015 OR dist_nfdb_year_2 < second_stand_photo_year) AND (dist_nfdb_year_3 IS NULL OR dist_nfdb_year_3 > 2015 OR dist_nfdb_year_3 < second_stand_photo_year) AND
...
------------------------------------------------------------------------------------------------------
---3.6b. Update calculation of age2015_nologging
------------------------------------------------------------------------------------------------------
...
CASE
    WHEN (
	(dist_year_wnw >= 1990 AND dist_year_wnw <= 2015 AND dist_type_wnw LIKE 'CUT') OR
	(dist_year_canlad >= 1990  AND dist_year_canlad <= 2015 AND dist_year_canlad =2) OR
	(dist_casfri_year_1 >= 1990 AND dist_casfri_year_1 <= 2015 AND (dist_casfri_type_1 LIKE 'CUT' OR dist_casfri_type_1 LIKE 'PARTIAL_CUT')) OR
	(dist_casfri_year_2 >= 1990 AND dist_casfri_year_2 <= 2015 AND (dist_casfri_type_2 LIKE 'CUT' OR dist_casfri_type_2 LIKE 'PARTIAL_CUT'))
	) THEN age_1990 +25
	  ELSE age_2015
END